    ...
    def evaluate(network, params, rng):
        ...
        def run_episode(rng):
            ...
            def step_fn(carry, unused):
                ...
                logits, value = network.apply(params, obs[None], training=False, rngs={"dropout": dropout_rng})
                action = jnp.argmax(logits[0])

                # Step environment
                rng, step_rng = jax.random.split(rng)
                next_obs, next_state, reward, next_done, info = env.step(step_rng, state, action, env_params)
            ...
            (_, final_state, _, _), (rewards, dones) = jax.lax.scan(
                step_fn, (obs, state, rng, jnp.array(False)), None, length=max_steps
            )
        ...
        def update_step(runner_state, update_idx):
            ...
            def _env_step(runner_state, unused):
                ...
                logits, value = network.apply(
                    train_state.params, last_obs, training=True, rngs={"dropout": dropout_rng}
                )
            ...
            def _update_epoch(update_state, unused):
                def _update_minibatch(update_info, batch_info):
                    ...
                    def _loss_fn(params, traj_batch, gae, targets, dropout_rng):
                        ...
                        if pretrained_params is not None and config.get("KL_COEF", 0.0) > 0:
                            ...
                            pretrained_logits, _ = network.apply(
                                pretrained_params, traj_batch.obs, training=False, rngs={"dropout": dropout_rng}
                            )
                ...
                (train_state, rng), loss_info = jax.lax.scan(_update_minibatch, (train_state, rng), minibatches)
        ...
        def _update_step(runner_state, unused):
            ...
            def _update_epoch(update_state, unused):
                def _update_minibatch(update_info, batch_info):
                    ...
                    grad_fn = jax.value_and_grad(_loss_fn, has_aux=True)
                    rng, dropout_rng = jax.random.split(rng)
                    total_loss, grads = grad_fn(train_state.params, traj_batch, advantages, targets, dropout_rng)
                    train_state = train_state.apply_gradients(grads=grads)
                ...
                (train_state, rng), total_loss = jax.lax.scan(_update_minibatch, (train_state, rng), minibatches)
                update_state = (train_state, traj_batch, advantages, targets, rng)
...
def make_train_with_callback(config, env, env_params, progress_callback):
    ...
    def train(rng):
        ...
        def _update_step(runner_state, update_idx):
            ...
            train_state, env_state, last_obs, rng = runner_state
            rng, dropout_rng = jax.random.split(rng)
            _, last_val = network.apply(train_state.params, last_obs, training=False, rngs={"dropout": dropout_rng})
